...
| Duplicate suppression | replay same idempotencyKey | FOUND DEFECT: second cancel returned already_terminal instead of idempotent; duplicate approval same | FIXED: added store.find_idempotent pre-check in _cancel; removed short-circuit pre-checks in approval/cancel so store idempotency layer decides |
| Operator identity binding | spoof operatorId in envelope | PROVEN: unauthorized | none |
| Rendering trust distinctions | untrusted model text vs authoritative state | PROVEN: [AUTHORITATIVE] tag separates them | none |
...
- Code corrected (3 genuine defects: terminal-state naming, inner idempotency keys,
  authority kind for planned task). Each fixed with a smallest change and re-verified.
- Tests added for each adversarial case (duplicate cancellation, conflicting/stale/expired
  approval, scope widening, spoofing, unauthenticated).
- Architecture notes updated (ARCHITECTURE, COMMAND_MAP, AUTHORITY_AND_IDENTITY, ADR-001).
- Evidence manifest records exact SHAs and verification outcomes.

## Residual risk (disclosed, not hidden)

- Single-user macOS operator model only; no multi-user/tenant isolation (documented).
- Acceptance DriverRun seeded via authoritative RuntimeService (real CAPT state path) rather
  than a live external driver, to stay harmless/deterministic; cancellation traverses the
  same governed path.
