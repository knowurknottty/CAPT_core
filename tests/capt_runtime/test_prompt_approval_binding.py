...
def test_runtime_builds_prompt_approval_receipt_for_exact_model_assembly(tmp_path):
    ...
    assert result["requestId"] == "r-model-1"
    assert result["missionId"] == "m-model-1"
    assert result["taskId"] == "t-model-1"
    assert result["driverRunId"] == "dr-model-1"
    assert result["promptAssemblyDigest"].startswith("sha256:")

    state = store.require_state("human_approval-r-model-1")
