...
def test_adopted_promotion_is_checkpoint_bounded_and_replay_equivalent(tmp_path):
    ...
    svc.adopt_artifact_promotion(
        "p-1", _meta("exec", "execution_plane", "6-adopt")
    )
    ...
    assert manifest["artifactPromotionVersions"] == [
        {"streamId": "artifact_promotion-p-1", "version": 3}
    ]
    full = full_replay(store)
    checkpointed = checkpoint_replay(store, manifest)
    assert checkpointed.aggregates["artifact_promotion-p-1"]["state"] == "adopted"
