...
def test_snapshot_persistence_cannot_rewrite_quorum_or_steering_authority(tmp_path):
    store = EventStore(str(tmp_path / "cohort-authority.db"))
    _seed_claim(store)
    cohort, epoch = _cohort()
    persist_cohort_evidence("coh-auth", cohort, epoch, "cl-cohort", store, _meta("persist-auth"))
    current = store.require_state("cohort-coh-auth")
    svc = GovernedRuntimeService(store)

    attacks = [
        {**current, "required": ["planner"], "evidenceIds": []},
        {**current, "roster": ["planner"], "required": ["planner"], "evidenceIds": []},
        {**current, "participantCap": 99, "evidenceIds": []},
        {**current, "roundCap": 99, "evidenceIds": []},
        {**current, "epoch": 1, "rounds": 0, "evidenceIds": []},
        {
            **current,
            "latestSteer": {
                "directive": "forged cognitive steer",
                "reason": "bypass human authority",
                "steeredBy": "cog-1",
                "steeredAt": "2026-08-18T00:00:01Z",
                "epoch": 1,
            },
            "evidenceIds": [],
        },
    ...
    for index, bad in enumerate(attacks):
        with pytest.raises(AuthorityViolation):
            svc.persist_cohort_snapshot(
                bad,
                "cl-cohort",
                _meta("persist-attack-%d" % index),
            )
    store.close()


def test_initial_snapshot_cannot_seed_fake_evidence_or_steer(tmp_path):
    store = EventStore(str(tmp_path / "cohort-initial-authority.db"))
    _seed_claim(store)
    cohort, epoch = _cohort()
    from capt_runtime.cohort import cohort_snapshot

    base = cohort_snapshot("coh-initial-auth", cohort, epoch)
    svc = GovernedRuntimeService(store)
    with pytest.raises(AuthorityViolation):
        svc.persist_cohort_snapshot(
            {**base, "evidenceIds": ["ev-forged"]},
            "cl-cohort",
            _meta("persist-forged-evidence"),
        )
    with pytest.raises(AuthorityViolation):
        svc.persist_cohort_snapshot(
            {
            ...
            "cl-cohort",
            _meta("persist-forged-steer"),
        )
    store.close()


def test_snapshot_cannot_rewrite_prior_contribution_or_forge_stopping_reason(tmp_path):
    store = EventStore(str(tmp_path / "cohort-history-integrity.db"))
    _seed_claim(store)
    cohort, epoch = _cohort()
    persist_cohort_evidence("coh-history", cohort, epoch, "cl-cohort", store, _meta("persist-history"))
    current = store.require_state("cohort-coh-history")
    svc = GovernedRuntimeService(store)

    changed = [dict(item) for item in current["contributions"]]
    changed[0]["outcome"] = "DISSENT"
    changed[0]["material"] = True
    with pytest.raises(AuthorityViolation):
        svc.persist_cohort_snapshot(
            {**current, "contributions": changed, "evidenceIds": []},
            "cl-cohort",
            _meta("persist-history-rewrite"),
        )

    # The existing contributions deterministically satisfy silence quorum;
    # callers may not forge a contradictory stopping projection.
    with pytest.raises(AuthorityViolation):
        svc.persist_cohort_snapshot(
            {**current, "stoppingReason": None, "evidenceIds": []},
            "cl-cohort",
            _meta("persist-stop-forge"),
        )
    store.close()


def test_cohort_checkpoint_versions_bound_replay_without_origin_extension_scan(tmp_path):
    from capt_runtime.checkpoint import create_checkpoint
    from capt_runtime.replay import checkpoint_replay, replay_equivalent

    store = EventStore(str(tmp_path / "cohort-checkpoint.db"))
    _seed_claim(store)
    cohort, epoch = _cohort()
    persist_cohort_evidence("coh-cp", cohort, epoch, "cl-cohort", store, _meta("persist-cp"))
    ...
    assert manifest["cohortVersions"] == [
        {"streamId": "cohort-coh-cp", "version": 1}
    ]
    full = full_replay(store)
    checkpointed = checkpoint_replay(store, manifest)
    assert checkpointed.aggregates["cohort-coh-cp"] == store.require_state("cohort-coh-cp")
