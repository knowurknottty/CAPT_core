...
def test_verified_and_accepted_remain_distinct_domain_scoped_states():
    claim = {
        "claimId": "cl-1",
        "statement": "Artifact digest matches expected output.",
        "evidenceIds": ["ev-1"],
...
def test_contradiction_never_renders_as_accepted_or_verified():
    claim = {
        "claimId": "cl-2",
        "statement": "Mutation was absent.",
        "evidenceIds": ["ev-2"],
        "verificationStatus": "contradicted",
        "promotionState": "rejected",
    }
    verification = {
        "status": {"kind": "contradicted", "domain": "effect_occurrence"},
        "committed": True,
    }
    item = project_claim_epistemic_state(claim, verification)
...
def test_advisory_observation_is_explicitly_not_committed_verification():
    claim = {
        "claimId": "cl-3",
        "statement": "Provider returned candidate text.",
        "evidenceIds": [],
        "verificationStatus": None,
        "promotionState": "proposed",
    }
    verification = {
        "status": {"kind": "observed_unverified"},
        "domain": "provider_output",
        "committed": False,
        "advisory": True,
    }
    item = project_claim_epistemic_state(claim, verification)
...
def test_multiple_claims_stay_separate_and_stale_only_when_source_marks_stale():
    ...
    ladder = project_epistemic_ladder(
        claims,
        {
