...
class _FakeClient:
    def identity(self):
        return {"integrity": "ok", "headSequence": 42, "ledgerChainDigest": "sha256:" + "a" * 64}
...
def test_dashboard_preserves_verifications_by_claim_and_builds_ladder(monkeypatch):
    claims = [
        {"claimId": "cl-a", "statement": "A", "evidenceIds": ["ev-a"], "verificationStatus": "verified", "promotionState": "accepted"},
        {"claimId": "cl-b", "statement": "B", "evidenceIds": ["ev-b"], "verificationStatus": "contradicted", "promotionState": "rejected"},
    ]
    verifications = {
        "cl-a": {"status": {"kind": "verified"}, "domain": "artifact", "committed": True},
        "cl-b": {"status": {"kind": "contradicted"}, "domain": "artifact", "committed": True},
    }
    monkeypatch.setattr(runtime_module, "project_authoritative_state", lambda client: {
        "missions": [], "tasks": [], "driverRuns": [], "claims": claims,
        "eventTimeline": [], "verificationsByClaim": verifications, "identity": client.identity(),
    })
