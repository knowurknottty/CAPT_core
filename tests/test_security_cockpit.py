...
def test_current_capt_core_empty_evidence_keeps_paid_service_control_blocking(tmp_path):
    import json
    ...
    profile = load_profile(root / "security" / "profiles" / "capt-core.json")
    evidence = load_evidence(root / "security" / "profiles" / "capt-core-evidence.json")
    gate = evaluate_security_gate(profile, evidence, source_sha="current-source-sha")
    cockpit = project_security_cockpit(gate.to_dict())

    assert len(cockpit["controls"]) == 47
    assert cockpit["counts"] == {
        "pass": 0,
        "fail": 0,
        "not_verified": 21,
    ...
    assert "CAPT-SUP-07" in cockpit["blockingControls"]
    assert cockpit["gateDecision"] == "BLOCKED"
    assert cockpit["releaseAuthorized"] is False
