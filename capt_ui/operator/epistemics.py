...
def project_claim_epistemic_state(
    claim: Mapping[str, Any], verification: Optional[Mapping[str, Any]] = None
) -> Dict[str, Any]:
    ...
    if verification.get("stale") is True or str(freshness).lower() == "stale":
        stages.append("STALE")

    if verification.get("committed") is True:
        provenance_class = "COMMITTED_VERIFICATION"
    elif verification.get("advisory") is True:
...
def project_epistemic_ladder(
    ...
    return [
        project_claim_epistemic_state(claim, by_claim.get(str(claim.get("claimId") or ""), {}))
        for claim in claims
    ]
