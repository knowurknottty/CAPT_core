...
struct PromptProposalCard: View {
    ...
    private var canSelect: Bool {
        proposal.isActive && proposal.status == "ready_for_approval" && !isBusy
    }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            Label("Prompt Intelligence proposal", systemImage: "brain.head.profile")
                .font(.headline)
            HStack(spacing: 8) {
                Text(proposal.status.replacingOccurrences(of: "_", with: " ").uppercased())
                    .font(.caption.bold())
        ...
        if !proposal.verificationCriteria.isEmpty || !proposal.unresolvedQuestions.isEmpty {
            VStack(alignment: .leading, spacing: 6) {
                if !proposal.verificationCriteria.isEmpty {
                    Text("Verification contract").font(.caption.bold())
                ...
                if !proposal.unresolvedQuestions.isEmpty {
                    Text("Unresolved before approval").font(.caption.bold())
