        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let item = selectedProvider {
                if item.id == "openrouter" {
                    Text("Set the OpenRouter API key once. CAPT stores it in macOS Keychain as service ‘capt-provider’, account ‘openrouter’; RuntimeService persists only the reference keychain:openrouter.")
                        .font(.callout)
