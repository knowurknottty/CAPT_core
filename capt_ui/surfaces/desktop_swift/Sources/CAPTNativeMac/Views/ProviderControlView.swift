        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let item = selectedProvider {
                if item.id == "openrouter" {
                    Text("Set the OpenRouter API key once. CAPT stores it in macOS Keychain as service ‘capt-provider’, account ‘openrouter’; the runtime persists only keychain:openrouter.")
                        .font(.callout).foregroundStyle(.secondary)
                    HStack {
                        ...
                        Button("Set API Key") {
                            ...
                            Task {
                                if await store.configureProviderAPIKey(providerID: item.id, apiKey: submittedKey) {
                                    apiKey = ""
                                }
                            }
                        }
                        .disabled(apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || store.isBusy)
                    }
                ...
                DisclosureGroup("Advanced credential reference", isExpanded: $showAdvancedCredentialReference) {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            ...
                            .disabled(credentialReference.isEmpty || store.isBusy)
                        }
