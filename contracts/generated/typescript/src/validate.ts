...
function checkRule(rule: RuleT, value: Json, path: string, out: string[]): void {
  ...
  switch (rule.t) {
    ...
    case "const":
      if (value !== rule.const) {
        err(out, path, `must equal ${JSON.stringify(rule.const)}`);
      }
      return;
...
function checkType(typeName: string, value: Json, path: string, out: string[]): void {
  ...
  if (entry.kind === "union") {
    ...
    err(
      out,
      path,
      `invalid discriminant ${disc}=${pyRepr(actual)}; expected one of ${allowed}`,
    );
    return;
  }
...
/** Render a value the way Python's %r does, so error strings match byte-for-byte. */
