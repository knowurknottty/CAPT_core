...
# Sentinel distinguishes "no baseline supplied" from a captured baseline of
# None (meaning the target was not a Git repository before dispatch).
...
def verify_repository_unchanged(
    ...
    The caller must capture ``before_digest`` BEFORE the governed driver is
    dispatched; capturing it afterward makes this check vacuous.
    """
...
def capture_git_status(target_path: str) -> Optional[str]:
    """Capture the exact porcelain Git status for delta verification.

    ``None`` means Git is unavailable or the target is not a Git repository.
    An empty string is a real, captured clean-repository baseline.
    """
    try:
        res = subprocess.run(
...
def verify_no_git_mutation(
    ...
    """Confirm the driver introduced no Git-status delta.

    When a pre-dispatch baseline is supplied, pre-existing dirt is allowed and
    only a change relative to that baseline fails verification.  When no
    baseline is supplied this preserves the legacy M0-B behavior and requires
    an otherwise-clean Git worktree.
    """
    ...
    if before_status is None:
        # Target was not a Git repository before dispatch.  If it became one,
        # that is itself a driver-created Git-state mutation.
        return after is None
    if after is None:
        # A repository that existed before dispatch disappeared or became
        # unreadable as Git state: conservatively classify as mutation.
        return False
    return after == before_status
...
def build_verification_result(
    ...
    The tree digest and, when supplied, Git status are expected to have been
    captured BEFORE driver dispatch.  This makes repository verification a
    delta check rather than an absolute cleanliness check.

    This is CAPT-authored (trust=capt_authoritative), not driver output.
    View-only annotations live under ``_view`` and must be stripped before
    contract validation or EventStore commit.
    """
    import time as _time
    ...
    evidence_ids = supporting_evidence_ids or [
        "ev-" + digest({"artifact": artifact_digest})
    ]
    record: Dict[str, Any] = {
        "schemaVersion": "1.0.0",
        "verificationId": "vr-" + digest({"t": target_path, "a": artifact_digest}),
        "claimId": claim_id,
...
def build_contradicted_verification_result(
    ...
    """Build a contract-valid persisted negative VerificationResult.

    ``contradicting_evidence_ids`` must refer to evidence preserved for the
    failed verification path.  The optional reason is view metadata only;
    frozen VerificationResult contracts intentionally keep status compact.
    """
    import time as _time
