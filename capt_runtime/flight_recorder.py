...
def export_flight(
    ...
    *,
    ...
    for stream_id, kind, version in store.all_aggregates():
        aggregate_inventory.append({"streamId": stream_id, "kind": kind, "version": version})
    ...
    members: Dict[str, bytes] = {
        ...
        "aggregates.json": _json_bytes({"inventory": aggregate_inventory, "states": aggregate_states}),
        "runtime_metadata.json": _json_bytes(
            redact(dict(runtime_metadata or {}), secret_keys=custom_secret_keys, secret_values=explicit_secret_values)
        ),
        "artifact_refs.json": _json_bytes(
            redact(list(artifact_refs or ()), secret_keys=custom_secret_keys, secret_values=explicit_secret_values)
        ),
    }
    if checkpoint_manifest is not None:
        members["checkpoint.json"] = _json_bytes(
            redact(dict(checkpoint_manifest), secret_keys=custom_secret_keys, secret_values=explicit_secret_values)
        )
