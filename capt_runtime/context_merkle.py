...
def build_context_merkle(pack: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    for identity in COMPONENT_ORDER:
        leaves.append(
            {
                "identity": identity,
                "digest": digest({"identity": identity, "payload": payloads[identity]}),
            }
        )
...
def diff_context_merkle(before: Mapping[str, Any], after: Mapping[str, Any]) -> Dict[str, Any]:
...
def build_prompt_prefix_plan(
    sections: Sequence[Mapping[str, Any]], *, breakpoint_after: Optional[str] = None
) -> Dict[str, Any]:
    """Model exact serialized-prefix identity separately from Merkle provenance."""
    ...
    rendered_prefix = "\n\n".join("[%s]\n%s" % (item["identity"], item["text"]) for item in prefix_sections)
    full_render = "\n\n".join("[%s]\n%s" % (item["identity"], item["text"]) for item in normalized)
    return {
        "schemaVersion": MERKLE_SCHEMA_VERSION,
