...
|---------------|--------|---------|-----------------|---------------|-------------------|--------|-----------|------------|-------------------|-------|------|------|-----------------|-------------------|----------------------|
| MemoryEngine | mem | SQLite-backed memory store (memories, tags, nodes, edges, aliases, conflicts, retention, lifecycle transitions) | ✅ `memory/engine.py` (1364 LOC, 32 tables) | `capt_solo/memory/engine.py` | — | complete | `MemoryEngine` in `api.py` | SQLite, SCHEMA_VERSION=4 | local-only, secret-screened on store | ✅ test_memory.py, test_v02_models, test_v03_* | ✅ DATA_MODEL, API | sqlite3, core.config | forward-migrates <4 | none (verify gates after ctp fix) | — |
