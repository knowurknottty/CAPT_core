...
- Live test of 8 fixtures (AWS access-key identifier, GitHub personal-access-token format, Slack bot-token prefix, Stripe live secret-key prefix, generic bearer token, RSA private-key block, lowercase password assignment, lowercase api-key assignment): ALL refused at CAPT boundary (`UnsafeConfiguration`). **The Slack bot-token prefix and Stripe live secret-key prefix were NOT refused by upstream alone** — CAPT-side `is_sensitive_input` was strengthened with high-precision patterns (Slack bot-token shape, Stripe live/test secret-key shape) to close the gap. False positives (benign arch text, `api_key endpoint`, normal text, an AWS-style identifier inside a word) pass through. Tests: `test_R7_refusal_policy_enforced_capt_and_upstream`, `test_R7_false_positives_not_refused`.
