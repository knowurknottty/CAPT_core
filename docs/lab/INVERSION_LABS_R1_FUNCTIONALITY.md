...
| Gap analysis | READY | advisory | Distinguishes exact `text_match_found`, co-located `related_text_found`, distributed `partial_text_evidence`, and zero-token `not_observed`; token evidence uses exact tokens plus conservative `s`/`es` inflections rather than arbitrary substrings; all states are lexical evidence only, never “implemented” |
