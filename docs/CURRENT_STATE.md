...
Protected `main` remains the published integration baseline. The runtime/product baseline inspected for this reconciliation was `24cd0a0adea0b54990f94776acd01610590c10c6`, after merged PR #115 and documentation-only PR #121. PR #122 is this current-state documentation reconciliation; when it is present on `main`, it advances the Git commit without changing the runtime/product semantics described by that baseline. Resolve the exact current `main` commit from Git rather than treating a prose snapshot SHA as permanently current.

`main` is not the authority for the newer terminal convergence work until PR #117 is actually merged.

For the live pull-request map, see [`PR_TOPOLOGY.md`](PR_TOPOLOGY.md). That document distinguishes terminal Core convergence, separate edition work, public-release planning, benchmark/probe work, and historical/review PRs.

### 3. Terminal convergence candidate

The terminal candidate is **PR #117**, branch `integration/capt-core-terminal-convergence-r2`, exact current head `33e24146094242d7a88612cea39267ef52a1d2e1`. It is the semantic reconciliation point for the formerly stacked Core implementation branches rather than a mechanical mega-merge.
...
- closed-unmerged PR #118 provider/model-coherence semantics reconciled into PR #117 rather than treated as a separately merged authority;
